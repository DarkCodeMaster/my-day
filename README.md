...
所有数据存储在 localStorage（key：`myday`），当前 schema 版本为 **v3**。状态为模块级单例，自动保存 watch 在模块加载时注册一次。
...
```jsonc
{
  ...
  "weightUnit": "kg",            // 体重单位偏好（kg/jin，v3 新增）
...
- v1 → v2：`moneyItems` 的旧字段 `plan` → `description`；旧 `todayLogs` 中的灵感提取到独立的 `inspirations` 数组；日志缺失 `date` 时补为当天；过滤早期示例数据
- v2 → v3：补 `weightUnit` 字段（默认 `kg`）
- 缺失字段一律用默认值补齐

导入更高版本（`version > 3`）的数据会在控制台警告并尽力加载。
